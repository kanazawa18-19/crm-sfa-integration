...
総期限共有の製品修正は `695f850` で完了。本書の実試験結果は修正前の記録。
[追加実サービス検証計画](sync_capacity_additional_trial_plan.md) に、12プロセス・4jobの
限定試験案と予算・必要実装・停止条件を整理した。計画のみで再試験は未実施。

## 競合6の実診断（最新の実試験・2026-09-10）

本人が短期認証更新/キーチェーン保存・scope6試験1回・observer観測1回を具体的に承認後、
独立QAが `acaaa24`（製品 `4dc00db`）で実行。
[独立実行証跡](sync_capacity_stage_six_result.json)。26.977秒で不合格、12子中11正常終了・1失敗。

| 診断項目 | 実測値 | 解釈 |
|---|---|---|
| 例外型/発生場所 | FailedPrecondition / rpc_commit | SDKの保存呼出し境界で条件不一致 |
| 試行回数 | 1 | 当該_atomicの初回試行。再試行0回 |
| 経過/時間切れ | 3181ms / true | 失敗捕捉時の再試行判断で3秒を超過 |
| 段階 | commit | 最後の失敗は保存段階 |
| 回数上限/非競合連鎖 | false / false | 8回上限や不確定な例外連鎖による停止ではない |

3181msは読取り・試験ガードの追加読取り・台帳待機・保存等を含む当該_atomic全体。
保存RPC単独の遅延、scope5も同原因、サービス全体の安全性は断定しない。
独立SECもコード/証跡を照合し、この範囲の診断を確認した。

observer事後観測は1回成功（2RPC/read102、書込み予約増分0）。合成12件=processing3/pending9、
3枠占有、owner/slot/attempts一致。試験終了直後・観測後とも試験Python0。
全12子の正常終了条件は満たさず、3枠を守ったことだけで合格へ変更しない。

| 累計予約 | 実行前 | 観測後 | 今回増分 |
|---|---:|---:|---:|
| RPC | 1227 | 1483 | 256 |
| 文書read | 3887 | 4438 | 551 |
| 文書write | 358 | 423 | 65 |
| runtime秒 | 5065 | 5785 | 720 |
| 文書名 | 77 | 90 | 13 |

これは請求実数ではなく安全側の予約累計。元created_at1788983820・期限9/24 04:57 JST、
stage2/推定拘束8 USD、Neon接続5/SQL24を保持。実費は未確定。RPC残517、文書名残10。
全scope1～6使用済み。成功2/3、不合格1/4/5/6で5回成功条件は未達。
新scope+12jobには13文書名必要で、残10では同型試験を追加できない。
台帳再作成・使用済scope再試行・枠解放/回収・削除・本番配備は行っていない。

ローカルで「初回の確定競合が3秒超で返ると再試行0」を再現し、
期限・最低再試行回数・呼出し元の時間制約を照合した。
[再現結果と修正設計](sync_capacity_retry_deadline_design.md)。独立QA100件成功。
次は総期限と初回再試行機会を分ける設計の実装。製品修正は未実施。
単なる上限延長や新scopeでの繰り返しは採らない。実サービスの追加試験は
文書名/元予算を保持する別の計画が必要。大規模/復旧/復元/同期outboxも残る。

## 固定診断と未使用scope6の準備（2026-09-10）

[診断設計・検証・停止位置](sync_capacity_failure_diagnostics.md)。発生場所と再試行終了条件を
記録するコード、元台帳限定の未使用scope6を準備した。独立対象215合格11skip。
本人承認後Gemini Pro/Claude Opus5中レビューを取得、`4dc00db`へ採否反映。
最終独立239合格11skip。scope6実行と短期token更新は自動承認レビューで開始前に拒否され、
具体的な実操作の本人確認待ち。scope6は未実行、元台帳は変更なし。
競合5の原因は未確定のまま。本番未配備。

## 競合5の実結果（2026-09-10・当時の記録）
