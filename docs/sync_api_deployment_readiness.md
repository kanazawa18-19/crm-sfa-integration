...
2026-09-09追記：`90fad5b`のコードとVercel設定を追加照合した。
シート行作成時は1処理で直接接続2本を保持する。DB枠に対応する全体の同時処理上限は
今回の設定取得でも確定できず、容量保証は未充足。
処理別根拠・計算例・必要な対策は[接続容量照合](sync_api_connection_capacity.md)に記録した。
