...
2. **夜間cronは1晩ぶんしか進まない**（上記「分割実行」参照）。`completed`と`SyncCursor`の
   ...
   `scripts/backfill_client_name_index.py`（ローカルのフル同期）の場合のみ、`synced_count`/
...
**【2026-08-28 実施済み】イベント全体の真のアトミック化**: 上記は「通知の記述を事実に合わせる」
対症療法だったが、その後**部分書き込みそのものを起こさせない再構成を行った**。
`Dispatcher.dispatch()`を次の3フェーズに分けている。

1. **スキーマ解決**（外部I/Oなし）。スキーマ未定義のプロパティをここで落とす
2. **現在値の取得**。イベント全体で必要なツールを洗い出し、**ツールごとに1回だけ**取得する。
   ここで`ApiError`/`RequestException`が出たら、**書き込みを1件も行わないままイベント全体を中止**する
3. **判定と書き込み**。取得済みのスナップショットだけを使い、以降は現在値の取得を行わない

副次的な効果として、**同じレコードへのAPI呼び出しがプロパティ数分から1回に減り**（5プロパティの
イベントなら5回→1回）、1イベント内の全プロパティが**同一スナップショット**を見るようになった。

懸念していたコンフリクト解決とSlack通知への影響は無かった。`resolve_conflict()`へ渡す
`candidates`の組み立ても、`notify_conflict`のプロパティ単位の粒度も、フェーズ3の中で従来どおり
プロパティ単位で行っているため。移行時に注意した点は次の2つ:

- **対象プロパティが1つも無い場合（全てスキーマ未定義）は取得フェーズごと飛ばす**。
  ここで取得しに行くと、以前は発生しなかったAPI呼び出しとその失敗を新たに生んでしまう
- **取得順の決定性**: 取得するツールは`_ALL_TOOLS`の順で回す（frozensetの反復順に依存しない）

回帰テスト: `test_dispatch_writes_nothing_when_fetch_fails_on_multi_property_event`
（複数プロパティのイベントで取得に失敗しても、どのツールにも1件も書き込まれないこと）と
`test_dispatch_fetches_each_tool_once_for_multi_property_event`（取得がツールごとに1回であること）。

**失われた更新の回収について（WARN4、運用上の残存リスク）**: アトミック化後も、
**現在値取得に失敗したイベントの更新は、送信元ツールで
