...
Firestore `sync_capacity_scopes/{scope}` にversion、limit、固定slotsのmapを保管し、`jobs/{hash}` に要求・状態・owner・試行数・時刻を保管する。jobとslotの更新は同一transaction。受付も共有設定をtransaction内で検証する。
...
- 初回claimを0〜0.5秒ずらし、Firestore transactionの確実なABORTEDだけ、最大8回・再試行開始予算3秒で短いランダム待ちを入れる。commit応答不明は再試行しない。なお競合でclaimが失敗した場合も要求・上限は保全されるが、空き枠を使い切れない回があり得る。定期drainが後で再取得する。実サービスでの競合率・十分な処理量は未検証。
...
- [Firestoreのトランザクション](https://firebase.google.com/docs/firestore/manage-data/transactions)：共有枠と要求を一括確定する根拠。外部の同期処理は再実行されるトランザクション関数に入れない。
...
2026-09-09に確認。gRPC例外の変換・Firestore transactionの再試行については、検証環境に導入したSDKの実装も照合した。

## 独立検証の最終結果（2026-09-09 主機CX）
...
**WARN：競合時の処理開始失敗は未解消。** 初版は同条件60回で取得8・空/満枠9・例外43、途中版は取得11・空/満枠27・例外22だったが、最終版は上表となった。改善を断言しない。最終例外の連鎖は`ValueError → Aborted(409) → grpc ABORTED`で、確実な競合として識別されている。再試行回数・時間予算の範囲内で成立しない回が残る。要求は次回に保管されるが、必要な流量を処理できる証明にはならず、配備保留条件とする。

実装者と分離したSEC・品質の最終レビューはBLOCKER 0 / WARN 0。QAの上記WARNは残す。Gemini Pro / Claude Opus 5・中での外部レビューは未実施。自動承認レビューが具体的な資料送信と、Claude新規画面に残る前回添付を外す操作を拒否したため、承認後に同じ資料で再開する。イシューは未完了で、本番配備しない。
