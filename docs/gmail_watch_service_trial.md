...
2026-09-08 主機CX。基準コード `40fd685`。本イシューの隔離検証は完了。未検証の範囲は各節に明記する。
...
| 監視 | 監視6条件の異常通知と停止後の復旧通知を受信確認。最終8スレッド10通。試験後はScheduler PAUSED・全6条件無効 |
...
実Gmail/実PostgresとPub/Sub、合成製品ログのCloud Logging到達、監視6条件/復旧通知、
検証SAの許可view200/権限未付与の空view403を確認し、本イシューを完了した。
専用Scheduler PAUSED・検証アラート全6条件無効・Postgres停止。検証Jobは1回完了し、定期起動なし。
実Gmailや通知試験を再runしない。本番配備・本番runは別イシューで本人の別途指示後。
...
3. 全viewへの誤付与：プロジェクトレベルの条件付き付与ではなく、専用viewリソースへの直接付与を
   実施した。付与後policyを再読取済み。権限なしの検証空viewへの拒否も後述の実測で確認。
...
| 終了欠落 | 開始・進捗のみ。猶予後missing_count1 | 18:01、18:06、18:12、18:40の4通を受信確認 |
| 監視エラー | 検証GRACE_SECONDSを一時invalid、/checkがconfiguration/500 | 18:04に受信確認 |
| 到達未確認 | 検証検索窓を一時2秒、entry_count0/HTTP200 | 18:11に受信確認 |
| 監視自体の停止 | 成功後に専用Scheduler停止 | 18:36に停止通知、18:42に復旧通知を受信確認 |
...
このJob作成・実行も当初は個別承認が必要として拒否されたが、本人の追加承認後に実施した。
検証viewの成功・権限設定の再読取・空view拒否を確認。継承権限の網羅確認は未実施。
...
組織/フォルダの継承権限全体を確認した証拠ではない。拒否実測は次項の検証空viewに限定する。

停止監視の実測：最終成功の非ゼロ値は09:11:30 UTC、最後の0値は09:13:30 UTC。
18:36 JSTのcheck_completed_missing発報メールを本人Gmailで受信確認した。
停止後の0値もデータ点として扱うため、停止操作から10分で届く保証とはしない。
集計窓600秒・欠落期間600秒に加えて、実測には評価/配送待ちが含まれる。
復旧通知の検証のため専用Schedulerを再開した。

### 停止・復旧の最終結果と片付け

再開後18:39/18:40 JSTのScheduler由来2要求はHTTP200、check_completedも2件、
指標の値1への復帰を独立QAが読取確認。各照合対象9件・終了未確認1件で、
監視が動いたことと業務の全件成功は別である。
18:42の本人Gmail通知本文にAlert recovered、同じcheck_missingポリシー、
継続時間5分20秒を確認。18:36停止通知と同じスレッド内の別メールだった。

最終検索の実測は8スレッド10通：一部失敗1、HTTP失敗1、終了欠落4、監視エラー1、
到達未確認1、監視停止1、復旧1。途中集計7通との差は停止・復旧と18:40の終了欠落再通知。
終了欠落の再通知は同じrevisionでも生じた。永続重複排除なしの限界を維持する。

試験後は専用SchedulerをPAUSED、登録した検証アラート6件をすべてenabled=falseへ戻した。
サービス・検証view・通知channel・検証IAM・実行証跡は保持。Postgresは停止済み。
本番配備・本番run・本番鍵再探索は実施していない。

### 閲覧拒否試験の最終結果

本人の「承認します」を受け、検証専用Job `crm-gmail-watch-trial-access-check` を作成した。
最初はメモリ256MiがGoogleの最低512Mi条件で作成拒否され、処理は未実行だった。
メモリだけ512Miに修正し、実行 `crm-gmail-watch-trial-access-check-fwzpr` が成功した。

- 専用SA、既存の検証イメージ固定digest、1タスク、再試行0、制限60秒。
- 同じ検証bucketの `_AllLogs` はHTTP200、権限未付与の空view `crm-trial-no-access` はHTTP403。
- 2026-09-08 09:56:12.749639 UTCのCloud Loggingに `access_trial: {allowed: 200, denied: 403}` が到達。
- 要求は固定の該当なし条件・最大1件。応答本文は読まず、認証値は出力しない。
- 本番_Defaultや他bucketへのアクセスは試験していない。この空viewで拒否された事実を、
  組織全体の権限の網羅検査と同一視しない。
- Jobは完了状態で実行証跡とともに保持。定期起動は作成していない。

独立SECがJob実行数1、成功1/失敗0/実行中0、完了09:56:15 UTC、専用SAと配備引数の
対象2view、結果ログ200/403をAPI再読取で確認。BLOCKER 0/WARN 0。再実行はしていない。
