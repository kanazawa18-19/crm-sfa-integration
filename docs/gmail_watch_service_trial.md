...
1. 残作業は、個別承認後の検証bucket内の許可view/権限未付与の空viewへのアクセス200/403確認。
   Google同意・検証IAM3点・本人宛通知の承認と設定は完了済みで、再確認不要。
2. 拒否試験の結果を追記してイシューを閉じる。Job作成・1回実行は自動審査に拒否され、本人確認待ち。
   専用Scheduler停止・検証アラート全6条件無効化は完了済み。
   実Gmail/実PostgresとPub/Sub、合成製品ログ到達、停止/復旧通知は検証済み。再runしない。
   本番配備・本番runは別途指示後。
