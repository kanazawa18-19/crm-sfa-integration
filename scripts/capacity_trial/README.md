...
専用資源 `cnctor-crm-cap-trial-260910` だけを許可する。製品のenvやADCを使わず、空HOME・環境変数許可制の子プロセスへ短命SA tokenを標準入力で渡す。秘密を引数・ファイル・ログに出さない。
...
台帳の予約値は請求操作数ではない。全件queryは最大10万読取りを予約し、実際に返却された文書を別に数える。索引読取り・サービス側再試行・請求遅延は費用の実測で照合する。強制killされたstreamの返却文書数は欠測になりうるが予約は保持する。費用の未測定を0として登録してはならない。
...
3. SA impersonationで取得した短命tokenをキーチェーンへ保管する。serviceは `capacity-trial-token-runner` または `capacity-trial-token-observer`、accountは対応する `capacity-trial-{role}@cnctor-crm-cap-trial-260910.iam.gserviceaccount.com`。
...
実Firestoreはproject作成上限により未接続。実IAM・複合index・永続性・料金をローカルテストで確認済みとは扱わない。
...
Neon Freeだけが作成済みの場合、追加課金が起こらないFree契約と残枠を実画面で確認し、その証跡に基づき `record-cost ... --usd 0 --cost-basis free-plan-verified` を登録できる。この0はメーターで実測した請求額ではなく、確認済み無料契約に基づく管理値。作成済み資源全体の追加課金有無が不明なら登録せず停止する。表示利用量の遅延を証跡に記録する。台帳のcreated-atは最初の試験資源作成日時を使う。
